...
### Syntax
```C
T hvx_vdeal(T x, chunk_size);
```
Temporarily considers x to be a 2xn block, and transposes x.
This is done irrespectively of the shape of x.
The shape of the result is the shape of x.

### Constraints
`x`'s shape can correspond to one or two HVX vectors.
