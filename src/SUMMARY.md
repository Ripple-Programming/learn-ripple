
**Ripple** is an API offering multi-level parallelism.

[Abstract](./abstract.md)
[License](./license.md)

# Specification

- [What is Ripple ?](./ripple-spec/ch1.md)
- [The SPMD programming model in Ripple](./ripple-spec/spmd.md)
- [Distributing loop computations in Ripple](./ripple-spec/loop-annotation.md)
- [Ripple API specification](./ripple-spec/api.md)
- [Calling functions in Ripple code](./ripple-spec/calling.md)
- [Ripple vector bitcode libraries](./ripple-spec/bc-lib.md)
- [Ripple vs. CUDA(R) and OpenCL(R)](./ripple-spec/vs-cuda.md)

# Optimization Guide

__Please note__: the Ripple HVX optimization guide is now a separate document.
- [Optimizing Ripple programs](./opt-guide/ch1.md)
- [SIMD optimization principles](./opt-guide/vector-principles.md)
- [Coalescing tips](./opt-guide/coalescing.md)

# Using Ripple

- [Getting started](getting-started.md)
<!--- [Release notes](release-notes.md) -->

<!-- # Integrated Development Environment
- [Installation](./dev-environment/dev-env.md)
- [Quick Start](./dev-environment/quick_start.md)
-->
---

CUDA is a trademark of NVIDIA Corporation.

OpenCL is a trademark of Apple Incorporated.

---
*Copyright (c) 2024-2025 Qualcomm Innovation Center, Inc. All rights reserved.
SPDX-License-Identifier: BSD-3-Clause-Clear*
